...
A feature document captures **iteration-level behavior** — a single feature's what and how in one file. 
...
- **Business rules** — Feature-specific rules (not shared; shared rules belong in `<domain>.md`)
- **Domain references** — Which entities, states, and domain rules from `<domain>.md` this feature leverages
...
- **No duplication of domain rules.** If a rule already exists in `<domain>.md`, reference it by ID. Do not restate it.
- **Promote when stable.** If a feature behavior becomes a domain invariant, move it to `<domain>.md`.
