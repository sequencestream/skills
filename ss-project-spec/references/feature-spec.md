...
Path: `domains/<group>/<domain>/features/<feature>.md`
...
- **Business rules** — Feature-specific rules (not shared; shared rules belong in `spec.md`)
- **Domain references** — Which entities, states, and domain rules from `spec.md` this feature leverages
...
- **No duplication of domain rules.** If a rule already exists in `spec.md`, reference it by ID. Do not restate it.
- **Promote when stable.** If a feature behavior becomes a domain invariant, move it to `spec.md`.
